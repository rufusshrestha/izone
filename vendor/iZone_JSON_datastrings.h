/*
 * Developer    : WM (Witold@iZone.com.au)
 ...
 * 1.11- added "AcUnitBrandSet" to system status
 *       added "SetAcUnitBrandSet" command
 * 1.12- added "Mode" and "Fan" to favourites info
 *       added "SchedAcMode" command
 *       added "SchedAcFan" command
 *       added "LockZoneNames" command and info in "SystemV2"
 *       added "UseInductEnergy" to info in "SystemV2" and "UseUnductEnergy" command (mind the difference)
 * 1.13- added "CpuType" to indicate whether C220 or C150 is connected
 *       added "WarningReset" command
 *       added "SysNo" to indicate system number (0 - 4)
 * 1.14- added AcCpuType_Coolbreeze to AcCpuType_e
 *       added SysMode_Exhaust and SysMode_PumpOnly to SysMode_e for Coolbreeze
 *       added Coolbreeze info to SystemV2
 *       added commands for changing Coolbreeze settings and paramenters
 * 1.15- added "CoolbreezeAutoFanMaxTime" command to Coolbreeze configuration
 * 1.16- added Ventialtion control option to universal unit controller
 *       added Brivis control to universion unit controller
 * 1.17- added "CoolbreezeDrCycEn" command
 *       added "CoolbreezeFavSettings" command
 * 1.18- added ventilation system control, configuraion and status
 * 1.19- added "AirPureOn" for controlling the Daikin air purifier
 * 1.20- added "crfs2SensorPair" command
 *-----------------------------------------------------------------------------
 * 
 */
...
//************************** DEVICE DISCOVERY *******************************//
// 
// CB/CL5B/COCB devices can be discovered on local LAN by sending "IASD" string
//  to to UDP bradcast address: 255.255.255.255:12107
// If the response is: 
// -	ASPort_12107,Mac_xxxxxxxxx,IP_xxx.xxx.xxx.xxx
// -	ASPort_12107,Mac_xxxxxxxxx,IP_xxx.xxx.xxx.xxx,iZone, ...
// Then this device only supports the original API, which is described in a
// different document
// 
// If the response is:
// -	ASPort_12107,Mac_xxxxxxxxx,IP_xxx.xxx.xxx.xxx,iZoneV2, ...
// Then this device also supprts the new API, described below. The support for
// the original API is maintained.
// 
// Example response:
...
// iZone - iZone air con support API v1 (described in different document)
// iZoneV2 - iZone air con support API v2 (described below)
...
// If a device does not support one of the above systems, the corresponding
// string will be replaced by "X" or missing
//
//***************************************************************************//

//***************** REQUEST STATUS INFORMATION EXAMPLE **********************//
//
// The endpoint for requesting information:
// \iZoneRequestV2
//
// - request system status information
//   {"iZoneV2Request":{"Type":1,"No":0,"No1":0}}
// - request zone 5 status information
//   {"iZoneV2Request":{"Type":2,"No":4,"No1":0}}
// 
// Check "iZoneV2Request" for details.
//
//***************************************************************************//

//*********************** SEND COMMANDS EXAMPLE *****************************//
//
// The endpoint for sending commands:
// \iZoneCommandV2
// 
// - turn system on
//   {"SysOn":1}
//
// - set zone 3 to climate control
//   {"ZoneMode":{"Index":2,"Mode":3}}
//
//
//***************************************************************************//
...
enum
{
  CoolbreezeControlSensor_Screen,
  CoolbreezeControlSensor_Remote
}CoolbreezeControlSensor_e;
 
 enum
 {
   AcCpuType_C220,
   AcCpuType_C150,
   AcCpuType_Coolbreeze
 }AcCpuType_e;
...
enum
{
  ...
  UnitBrand_YorkAc,
  UnitBrand_HaierAc
  UnitBrand_LG_Coolink,
  UnitBrand_BraemarAc,
  UnitBrand_Samsung_2,
  UnitBrand_RinnaiNetworker,
  UnitBrand_CoolbreezeEvap,
  UnitBrand_MideaSplitSystem
}UnitBrand_e;
...
enum
{
  ...
  GasHeatType_VentilationControl,
  GasHeatType_AA_GEN_III,
  GasHeatType_Brivis1HeatAnd1Cool_1SpeedFan,
}GasHeatType_e;
...
enum
{
  OemMake_Airstream,      // iZone
  OemMake_Metalflex,      // MyZone
  OemMake_Westaflex,      // 
  OemMake_Electrolux,     // Kelvinator
  OemMake_Consolidated,   //
  OemMake_Toshiba,        // T-Zone
  OemMake_AirAdditions,   // RayZone
  OemMake_CoolbreezeEvap  // coolbreeze evap
}OemMake_e;

enum
{  
  UnitBrandSet_Auto_Detect,
  UnitBrandSet_Panasonic,
  UnitBrandSet_Daikin,
  UnitBrandSet_Fujitsu,
  UnitBrandSet_Fujitsu_20kW,
  UnitBrandSet_Mitsubishi_Electric,
  UnitBrandSet_Mitsubishi_Heavy_Industries,
  UnitBrandSet_Hitachi,
  UnitBrandSet_Toshiba,
  UnitBrandSet_LG_Coolink,
  UnitBrandSet_Samsung,
  UnitBrandSet_Temperzone,
  UnitBrandSet_Universal_Control_Module,
  UnitBrandSet_Actron,
  UnitBrandSet_Braemar,
  UnitBrandSet_Haier,
  UnitBrandSet_Kaden,
  UnitBrandSet_Kelvinator,
  UnitBrandSet_Midea,
  UnitBrandSet_Rinnai,
  UnitBrandSet_York,
  UnitBrandSet_iZone,
  UnitBrandSet_Samsung_2,
  UnitBrandSet_Max
}UnitBrandSet_t;	 

enum
{
  Crfs2TxInterval_NotSet,
  Crfs2TxInterval_2s,
  Crfs2TxInterval_4s,
  Crfs2TxInterval_8s,
  Crfs2TxInterval_16s,
  Crfs2TxInterval_32s,
  Crfs2TxInterval_64s,
  Crfs2TxInterval_128s,
  Crfs2TxInterval_256s,
  Crfs2TxInterval_NoOf
}Crfs2TxInterval_e;
 ...
 
{
	...
	"SystemV2":	{
		...
		*"LockCode": string,				// up to 6 digits
		...
		"AirPureOn": int,					// turn on/off Daikin purifier
...
// b - is the TX interval: Crfs2TxInterval_e

// ----------------------- Ventilation system commands ------------------------

// change use ECO2 control
{
	"VentilationUseEco2Control":x
}
// where:
// x - is the new setting ( 0 (dont control ECO2) - 1 (control ECO2) )

// change use VOCs control
{
	"VentilationUseVcosControl":x
}
// where:
// x - is the new setting ( 0 (dont control VOCs) - 1 (control VOCs) )

// change use RH control
{
	"VentilationUseRhControl":x
}
// where:
// x - is the new setting ( 0 (dont control RH) - 1 (control RH) )

// change cycle fan off
{
	"VentilationCycleFanOff":x
}
// where:
// x - is the new setting (0 (dont cycle the fan) - 1 (cycle the fan) )

// change fan stage delay
{
	"VentilationFanStageDelay":x
}
// where:
// x - is the new delay in minutes (3 - 240)

// change ECO2 setpoint
{
	"VentilationEco2Setpoint":x
}
// where:
// x - is the new setpoint (500 - 1500ppm)

// change VOCs setpoint
{
	"VentilationVocsSetpoint":x
}
// where:
// x - is the new setpoint (50 - 2500ppb)

// change the RH (relative humidity) setpoint
{
	"VentilationRfSetpoint":x
}
// where:
// x - is the new setpoint (5 - 95%)

// --------------------------- Coolbreeze commands ----------------------------

// change coolbreeze favourite settings
{
	"CoolbreezeFavSettings":
	{
		"Index":x,
		"UnitSetpoint":y,
		"FanSpeed":z,
...
// y - is the unit setpoint (1500 - 3000)
// z - is the fan speed (1 - 100)
// w - is the RH setpoint (10 - 90%)


// change coolbreeze drain cycle enable
{
	"CoolbreezeDrCycEn":0
}
// where:
// x - is enable flag (0 - 1)

// change coolbreeze fan speed
{
	"CoolbreezeFanSpeed":x
}
// where:
// x - is fan speed (1 - 100)

// change humidity setpoint
{
	"CoolbreezeRhSetpoint":x
}
// where:
// x - is humidity setpoint (10 - 90)

// change pre wash enable
{
	"CoolbreezePrewEn":x
}
// where:
...
// change pre wash time
{
	"CoolbreezePrewTime":x
}
// where:
// x - is pre wash time (1 - 60 minutes)

// change drain after pre wash enable
{
	"CoolbreezeDrAfPrewEn":x
}
// where:
// x - is enable flag (0 - 1)

// change drain cycle period
{
	"CoolbreezeDrCycPer":x
}
// where:
// x - is pre wash time (1 - 3000 minutes)

// change post wash enable
{
	"CoolbreezePostwEn":x
}
// where:
...
// change drain before post wash enable
{
	"CoolbreezeDrBfPostwEn":x
}
// where:
// x - is enable flag (0 - 1)

// change post wash time
{
	"CoolbreezePostwT":x
}
// where:
// x - is post wash time (5 - 30 minutes)

// change inverter setting
{
	"CoolbreezeInverter":x
}
// where:
// x - is enable flag (0 - 1)

// change resume last state
{
	"CoolbreezeResumeLast":x
}
// where:
// x - is enable flag (0 - 1)

// change maximum fan speed in auto mode
{
	"CoolbreezeFanMaxAuto":x
}
// where:
// x - is fan speed (1 - 100)

// change maximum fan speed in manual mode
{
	"CoolbreezeFanMax":x
}
// where:
...
// change maximum fan speed in exhaust mode
{
	"CoolbreezeExhMax":x
}
// where:
...
// change controlling sensor
{
	"CoolbreezeCtrlSens":x
}
// where:
// x - is CoolbreezeControlSensor_e

// change temperature sensor calibration
{
	"CoolbreezeCalibTemp":x
}
// where:
// x - is calibration value (-50 - 50) /10 to get value

// change temperature dead band
{
	"CoolbreezeDeadTemp":x
}
// where:
// x - is dead band value (100 - 500) /100 to get value

// change auto mode fan maximum speed time
{
	"CoolbreezeAutoFanMaxTime":x
}
// where:
// x - is time in minutes (0 - 60)

// ------------------------------- configuration ------------------------------

// reset iZone warning command
{
	"ResetWarning":string
}
// where:
// string - is "filter" for filter warning reset

// Use induct energy command
{
	"UseUnductEnergy": x
}
// where:
// x - is the setting (0/1)
...
// where:
// x - AC unit brand to be set (UnitBrandSet_t)
...
//*********************** Favourites JSON **********************************//
{
	...
	"SchedulesV2": {
		...
		"Coolbreeze":
		{
			"UnitSetpoint":int,		// unit setpoint
			"FanSpeed":int,			// fan speed 1 - 100%
			"RhSetpoint":int			// RH setpoint 10 - 90%
		}
...
// where:
// a - is the schedule number (0 = favourite 1,...)
// b - the new mode (SysMode_e)
...
// d - required system mode (SysMode_e)
// e - required fan speed (SysFan_e)
